...
def get_anomaly_score(model, data, true_next_value):
    ...
    return (
        (pred_next_value - true_next_value.to(model.device))
        .abs()
        .cpu()
        .detach()
        .numpy()
    ...
    try:
        ...
        for epoch in trange(EPOCHS):
            ...
            if name == "all":
                ...
                print(
                    "Epoch {0} done\t train_loss={1} \t eval_loss={2}".format(
                        epoch, train_loss, eval_loss
                    )
                )
            torch.save(
                model.state_dict(),
                "models/deepant_{0}_epoch_{1}__eval_loss_{2}.pth".format(
                    name, epoch, eval_loss
                ),
            )
