...
def evaluate(pred_anomaly: pd.DataFrame, actual_anomaly: pd.DataFrame):
    ...
    """
    output = set(
        zip(list(pd.to_datetime(pred_anomaly.timestamp)), list(pred_anomaly.cell_name))
    )
    anomaly = set(
        zip(
            list(pd.to_datetime(actual_anomaly.timestamp)),
            list(actual_anomaly.cell_name),
        )
    )
...
def true_results(path, threshold):
    if ".pkl" in path:
        ...
        return df.loc[
            df.drop(columns=["timestamp", "cell_name"]).max(axis=1) >= threshold
        ]
    else:
